return {
	...
	opts = {
		servers = {
			...
			-- The following servers will be automatically set up by mason-lspconfig
			-- with default settings. Only servers needing specific custom configurations
			-- (like lua_ls above) should be listed here.
			-- tsserver = {},
			-- html = {},
			-- cssls = {},
			-- tailwindcss = {},
	...
	config = function(_, opts)
		...
		for server_name, server_config in pairs(opts.servers) do
			...
			if lspconfig[server_name] and lspconfig[server_name].setup then
				...
				vim.notify(
					"LSP server '" .. server_name .. "' not found in lspconfig. Skipping setup.",
					vim.log.levels.WARN
				)
			end
		end
